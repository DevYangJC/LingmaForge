...
> 这篇文章填补那个空白。它从"设计"走到"实现"——每个节点到底怎么工作、LangChain4j 的 `AiServices` 接口模式怎么替代手动 JSON 解析、文件怎么从大模型的输出变成磁盘上的真实文件、迭代修改怎么定位到该改的代码、提示词怎么存储和管理、需要编写哪些 `@Tool` 方法、用什么设计模式让整个系统可扩展。
...
- [二、节点一：需求分析——把一句话变成结构化蓝图](#二节点一需求分析把一句话变成结构化蓝图)
  - [2.6 结构化输出：用 `AiServices` 接口模式而不是手动解析 JSON](#26-结构化输出用-aiservices-接口模式而不是手动解析-json)
...
- [九、@Tool 工具体系——Agent 的手脚](#九tool-工具体系agent-的手脚)
...
**流水线的本质是六个 Agent 协作完成一个复杂任务。** 每个 Agent 有自己的"大脑"（LLM 模型）、自己的"手脚"（`@Tool` 方法）、自己的"记忆"（`AgentState` 状态）。它们不是简单的函数调用，而是自主的决策单元——模型决定了要调用哪个工具、工具返回了结果之后模型决定下一步做什么、模型认为任务完成了就停止循环。
...
每个节点里的 Agent 循环是"演员"——导演告诉它"你现在是需求分析阶段，拿到的是用户的原始 prompt"，演员自己决定怎么分析需求、要不要调用工具读一下项目上下文、最终输出什么格式的 JSON。导演只关心演员表演完了没有（节点执行结束），不关心演员表演的具体内容。
...
**注意：因为我们使用了 LangChain4j 的 `AiServices` 接口模式做结构化输出（详见第 2.6 节），提示词中不再需要写"只输出 JSON，不要输出其他内容"这样的硬约束。`AiServices` 内部会自动做 JSON Schema 生成 + 格式约束 + 反序列化，模型不会在 JSON 前面加解释性文字，也不会用 ```json``` 包裹输出。**这是 `AiServices` 相比手动解析的核心优势——你不需要在提示词里"教模型怎么输出 JSON"，框架替你做了这件事。**
...
因为 `AiServices.create(RequirementAnalyzer.class, model)` 会自动从 `RequirementSpec` 这个 Java 类生成 JSON Schema，注入到模型请求中。模型看到的请求里已经包含了完整的格式约束——每个字段的名称、类型、是否必需、描述信息。模型只需要按照 Schema 输出，框架自动把 JSON 反序列化成 `RequirementSpec` 对象。
...
需求分析节点理论上**不需要任何 @Tool 方法**。因为使用了 `AiServices` 接口模式做结构化输出（详见第 2.6 节），模型直接输出 JSON 并被框架自动反序列化——整个过程中模型不需要"动手"——它只需要"动脑"。
...
```java
// LangChain4j 的方式：普通类 + @Tool 注解，无需实现接口
public class ProjectContextTools {

    private final ProjectService projectService;

    @Tool("读取项目上下文信息，包括框架类型、已有文件列表、package.json 依赖")
    public String readProjectContext(String projectId) {
...
**为什么是可选的？** 因为首次生成时项目是空的，没有上下文可读。只有迭代修改时才需要这个工具。LangChain4j 的 `AiServices` 会根据需要自己决定是否调用——如果 system prompt 没有提到"先读项目上下文"，模型不会主动调用；如果提到了，模型会先调用再分析。

### 2.6 结构化输出：用 `AiServices` 接口模式而不是手动解析 JSON

这是这篇文章最关键的技术决策之一。很多人写 Agent 时习惯让模型输出 JSON，然后手动解析：

```java
// 旧方式：手动解析 + 容错
String rawOutput = chatModel.chat(prompt).aiMessage().text();
...
3. **没有自纠错能力**——解析失败了，代码只能抛异常或返回 null。你没法告诉模型"你的输出格式不对，请重新输出"。

**LangChain4j 的 `AiServices` 接口模式彻底解决了这三个问题。**

#### 核心理念：定义接口契约，框架自动实现

LangChain4j 的结构化输出方式是**声明式**的——你定义一个 Java 接口，指定返回类型，框架自动生成实现。返回类型就是结构化输出类型：

```java
// 1. 定义返回类型（POJO / Record）
...
// 5. 调用——返回值直接就是 Java 对象，零手动解析
RequirementSpec spec = analyzer.analyze("帮我生成一个会员订阅商城");
```

`AiServices` 内部做了三件事：

1. **自动生成 JSON Schema**：从 `RequirementSpec` 类的字段定义、`@Description` 注解生成完整的 JSON Schema。模型收到的请求里已经包含了"每个字段叫什么、类型是什么、是否必需、描述是什么"的约束。

2. **自动约束模型输出格式**：模型按照 JSON Schema 输出——不会加前缀文字、不会加注释、不会用 markdown 包裹。`strictJsonSchema(true)` 让模型在输出层就按 Schema 生成，而不是"先自由输出 JSON 再尝试匹配 Schema"。
...
需要返回 `List<XXX>` 这种泛型时，直接用接口返回类型指定即可：

```java
interface ExecutionPlanner {
    @SystemMessage("你是项目架构师，根据需求规范规划文件清单")
...
public record PlanResult(
    ...
    int totalPages,
    int totalApis,
    List<FilePlan> generationOrder
) {}
```

#### 流式场景的结构化输出

如果需要流式展示模型"思考过程"，可以使用 `TokenStream` 返回类型，但结构化输出不能直接流式。在需求分析和执行规划这两个节点，我们不需要流式——用户看到的是"分析中..."等待状态，完成后一次性展示结果。流式输出主要用在代码生成节点（逐字展示代码）。

#### 需求分析节点的最终实现

结合 LangGraph4j 的 `NodeAction` 和 LangChain4j 的 `AiServices`，需求分析节点的完整代码变成：

```java
@Component
public class RequirementAnalysisNode {

    private final RequirementAnalyzer analyzer;

    public RequirementAnalysisNode(
            @Qualifier("deepSeekModel") ChatModel deepSeekModel,
            PromptTemplateLoader promptLoader) {
        // 用 AiServices 创建接口实现
        this.analyzer = AiServices.builder(RequirementAnalyzer.class)
            .chatModel(deepSeekModel)
            .systemMessageProvider(chatMemoryId -> promptLoader.loadSystemPrompt("requirement-analysis"))
            .build();
    }

    /**
     * 作为 LangGraph4j 节点执行
     * 返回 Map<String, Object> → 更新 AgentState
     */
    public Map<String, Object> execute(CodeGenState state) {
        ...
        return Map.of("analysisResult", spec);
    }
}
```
...
| 变化点 | 旧方式（手动解析） | 新方式（AiServices） |
|--------|------------------|---------------------|
| 提示词 | 需要写 JSON 格式定义 + "只输出JSON"约束 | 只需要写角色定义和分析规则，`@SystemMessage` 注解 |
| Java 类 | 和提示词里的格式定义是两份独立的东西 | 是唯一的格式来源，提示词不再重复定义 |
| 解析代码 | `JsonOutputParser.extractJson()` + `JSON.parseObject()` | 不需要，`AiServices` 自动做 |
| 容错代码 | 去掉前缀文字、去掉 markdown 包裹、去掉注释 | 不需要，`strictJsonSchema(true)` 保证格式 |
| 纠错机制 | `validateJson` 工具 + 自纠错循环 | 不需要，框架级保证格式正确 |
| State 存储 | 存 JSON 字符串，读取时手动反序列化 | 直接存 Java 对象，类型安全 |
| 接口风格 | 命令式（拿 ChatClient → 调 .entity()） | 声明式（定义接口 → 框架自动实现） |

**最重要的结论：`AiServices` 让结构化输出从"提示词约定 + 手动解析 + 容错纠错"变成了"Java 接口定义 + 框架自动实现"。你不再需要维护两份格式定义，不再需要写容错代码，不再需要自建纠错循环。这是 LangChain4j 对 Agent 开发最大的贡献之一。**
...
1. **输入注入**：system prompt 不包含需求分析的 JSON——需求分析的结果是通过 AgentState 传递的 `RequirementSpec` 对象，在 user prompt 中动态拼接。
...
4. **不需要写 JSON 格式约束**：和需求分析节点一样，使用 `AiServices.create(ExecutionPlanner.class, model)` 自动约束输出格式。
...
**为什么不再需要"只输出 JSON"的约束？** 和需求分析节点同理——`AiServices.create(ExecutionPlanner.class, model)` 自动从 `PlanResult` Java 类生成 JSON Schema，框架自动约束模型输出格式。提示词只需要告诉模型"怎么规划"，不需要告诉模型"输出什么格式"。

### 3.5 执行规划节点的实现

执行规划节点和需求分析节点一样，使用 LangChain4j 的 `AiServices` 接口模式做结构化输出。不需要 Agent 循环（没有工具调用），也不需要手动 JSON 解析。

```java
@Component
public class ExecutionPlanningNode {
    ...
    public ExecutionPlanningNode(
            @Qualifier("deepSeekModel") ChatModel deepSeekModel,
            PromptTemplateLoader promptLoader) {
        this.planner = AiServices.builder(ExecutionPlanner.class)
            .chatModel(deepSeekModel)
            .systemMessageProvider(chatMemoryId -> promptLoader.loadSystemPrompt("execution-planning"))
            .build();
    }
    ...
    public Map<String, Object> execute(CodeGenState state) {
        ...
        // 需求分析结果作为 user prompt 注入
        String userPrompt = "请根据以下需求规范规划文件清单：\n" + analysisResult.toJsonString();

        // 一行搞定：模型输出 → PlanResult 对象，零手动解析
        PlanResult plan = planner.plan(userPrompt);

        return Map.of("planResult", plan);
    }
}
```

**为什么执行规划节点不需要 Agent 循环？**

因为它不需要调用任何工具——输入完全来自上游节点的 State（`analysisResult`），输出直接写入 State（`planResult`）。整个过程是"拿到需求分析结果 → 调用模型 → 得到规划结果"，没有"模型决定调用工具 → 工具返回结果 → 模型继续推理"的循环。所以用 `AiServices` 的单次调用模式就够了，不需要 `agent → tools → agent` 循环图。

### 3.6 需要什么工具？

执行规划节点**不需要 @Tool 方法**。它的输入完全来自 AgentState（需求分析的 `RequirementSpec` 对象），输出是 `PlanResult` 对象（直接写入 AgentState）。

但有一个可选的工具：**readTemplates**——读取灵码工坊内置的项目模板文件（比如标准的 package.json 模板、vite.config.ts 模板），让模型知道"标准的 React 项目配置是什么样的"。这能减少模型"发明"不标准配置的概率。如果需要这个工具，就得改用 `AiServices.builder()` 配合 `.tools()` 注册，让模型自己决定是否调用。
...
1. 从 AgentState 获取当前要生成的文件信息（path、type、description）
2. 从 AgentState 获取需求分析的结果（作为生成上下文）
...
代码生成节点不是"一次调用就完成"，而是**在 LangGraph4j 的 `agent → tools → agent` 循环图中循环**——模型决定调用 writeFile 工具写入一个文件，工具返回结果后模型继续推理，决定是否再调用 readFileContext 读取上下文、是否再调用 writeFile 写入下一个文件，直到模型认为所有文件都已生成完毕。

实现方式有两种：

**方案 A：LangGraph4j 的 `agent → tools → agent` 循环图**（推荐）

用 LangGraph4j 的 `StateGraph` 构建"模型节点 → 条件路由 → 工具节点 → 模型节点"的循环。模型自己决定生成顺序——system prompt 告诉它"有 14 个文件要生成，请逐文件调用 writeFile"。模型会先调用 readFileContext 读取上下文，然后输出代码内容，然后调用 writeFile 写入文件，然后继续下一个文件。LangGraph4j 的条件边 `routeMessage` 会根据模型输出是否包含工具调用来自动路由——有工具调用就进入 `invokeTools` 节点，没有工具调用就进入 END 节点。

```java
// 构建 agent → tools → agent 循环图
@Component
public class CodeGenerationGraph {

    private final StateGraph<CodeGenState> graph;

    public CodeGenerationGraph(
            @Qualifier("codeGenModel") ChatModel codeGenModel,
            ProjectContextTools projectTools,
            FileTools fileTools) {

        // 1. 创建带 @Tool 的 AiServices
        CodeGenAgent agent = AiServices.builder(CodeGenAgent.class)
            .chatModel(codeGenModel)
            .tools(projectTools, fileTools)  // readFileContext, writeFile, validateCode
            .build();

        // 2. 构建 StateGraph
        this.graph = new StateGraph<>(CodeGenState.class)
            .addNode("agent", callModel(agent))
            .addNode("tools", invokeTools(agent))
            .addConditionalEdges("agent", routeMessage(agent),
                Map.of("tools", "tools", END, END))
            .addEdge("tools", "agent")       // 工具执行完 → 回到模型
            .addEdge(START, "agent");         // 开始 → 进入模型

        this.graph.compile();
    }

    /** 模型节点：调用 agent 推理 */
    private NodeAction<CodeGenState> callModel(CodeGenAgent agent) {
        return state -> {
            String response = agent.chat(state.prompt().orElseThrow());
            return Map.of("messages", List.of(new AiMessage(response)));
        };
    }

    /** 路由节点：判断模型输出是否包含工具调用 */
    private EdgeAction<CodeGenState> routeMessage(CodeGenAgent agent) {
        return state -> {
            // 如果最后一条消息包含工具调用 → 路由到 tools
            // 如果最后一条消息是纯文本 → 路由到 END
            AiMessage lastMessage = state.lastAiMessage();
            return lastMessage.hasToolExecutionRequests() ? "tools" : END;
        };
    }

    /** 工具节点：执行工具调用 */
    private NodeAction<CodeGenState> invokeTools(CodeGenAgent agent) {
        return state -> {
            // LangChain4j 内部执行所有工具调用，返回 ToolExecutionResult
            List<ToolExecutionResult> results = agent.executeTools(state.lastToolRequests());
            return Map.of("messages", results.stream()
                .map(r -> new ToolExecutionResultMessage(r.toolName(), r.result()))
                .collect(Collectors.toList()));
        };
    }

    /** 执行代码生成循环 */
    public AsyncGenerator<NodeOutput> stream(CodeGenState initialState) {
        return graph.stream(initialState);
    }
}
```

`agent → tools → agent` 循环图的优势是**白盒透明**——你能看到每一轮循环中模型做了什么决策、调用了什么工具、工具返回了什么结果。如果模型在第 5 轨迹调用了一个不应该调用的工具，你可以从日志中清楚地看到，然后在提示词中调整。Spring AI Alibaba 的 `ReactAgent` 是黑盒——你只知道 Agent 执行完了，中间过程很难追踪。

**方案 B：外部循环 + 每次调用 ChatModel**

在 Java 代码里用 for 循环遍历 `generationOrder`，每次循环构建一个 prompt 调用 ChatModel，然后手动调用 writeFile。

```java
public Map<String, Object> execute(CodeGenState state) {
    List<FilePlan> plans = state.planResult().orElseThrow().generationOrder();
    for (FilePlan plan : plans) {
        // 构建 prompt
        String prompt = buildPromptForFile(plan, state);
        // 调用模型
        String response = codeGenModel.chat(prompt);
        // 写入文件
        fileTools.writeFile(plan.getPath(), response);
    }
    return Map.of("generatedFiles", plans);
}
```
...
**我推荐方案 A（LangGraph4j 的 `agent → tools → agent` 循环图）。** 因为代码生成是一个不确定性任务——模型需要根据每个文件的复杂度动态调整生成策略，确定性循环无法做到这一点。而且 LangGraph4j 的白盒模式让你能清晰地追踪每一轮 Agent 循环的决策过程，调试和优化都比 ReactAgent 的黑盒模式更方便。
...
**桥梁是 writeFile @Tool 方法。**

当代码生成 Agent 决定"我现在要生成 PlanCard.tsx"时，它调用 writeFile 工具：
...
2. **更新 AgentState**：把生成的文件信息写入状态——`generatedFiles` 列表新增一条记录 `{path, content, status: "new"}`。这样后续文件生成时可以读到这个文件的内容（通过 readFileContext 工具），样式优化节点可以知道有哪些文件需要微调 CSS。

```java
// LangChain4j 的方式：普通类 + @Tool 注解
@Component
public class FileTools {
    ...
    @Tool("将生成的代码内容写入项目文件，同时更新数据库和推送 SSE 事件")
    public String writeFile(String path, String content) {
        String projectId = getCurrentProjectId();  // 从 ThreadLocal 获取
        String fullPath = projectService.getProjectWorkspace(projectId) + "/" + path;
        ...
        // 2. 更新数据库
        projectService.saveProjectFile(projectId, path, content, "new");

        // 3. SSE 推送（通知前端文件树新增节点）
        emitFileGeneratedSse(path, content);

        return "文件写入成功: " + path + "（" + content.lines().count() + " 行）";
    }
}
```
...
```java
// LangChain4j 的方式：@Tool 注解方法，归属 ProjectContextTools 类
@Component
public class ProjectContextTools {

    private final ProjectService projectService;

    @Tool("读取项目中已生成文件的内容，用于提供生成上下文")
    public String readFileContext(List<String> paths) {
...
从 AgentState 获取。每次代码生成 Agent 进入 `agent → tools → agent` 循环时，State 里已经包含了：
...
```java
// LangChain4j 的方式：@Tool 注解方法，归属 FileTools 类
@Component
public class FileTools {
    ...
    @Tool("验证生成的代码质量。检查 import 路径、export 声明、TypeScript 类型完整性")
    public String validateCode(String path, String content) {
...
public class FileTools {
    ...
    @Tool("对项目文件做增量修改（diff patch），只修改指定行，不重写整个文件")
    public String patchFile(String path, List<Patch> patches) {
        String projectId = getCurrentProjectId();
        String fullPath = projectService.getProjectWorkspace(projectId) + "/" + path;
        ...
        for (Patch patch : patches) {
            int lineIndex = patch.line() - 1;  // 0-based index
            if (lineIndex >= 0 && lineIndex < lines.size()) {
                String currentLine = lines.get(lineIndex);
                if (currentLine.trim().equals(patch.old().trim())) {
                    lines.set(lineIndex, patch.new());
                }
            }
        }
        ...
        // 更新数据库（记录为 modified）
        projectService.saveProjectFile(projectId, path,
            String.join("\n", lines), "modified");

        return "样式优化完成: " + patches.size() + " 处修改已应用到 " + path;
    }
}
```
...
3. 根据构建结果更新 AgentState：`buildStatus = SUCCESS` 或 `FAILED`
...
```java
// LangGraph4j 条件边定义（使用 AgentState + Channel + Reducer）
...
当流水线回退到 codeGeneration 节点时，AgentState 里已经有了：
...
**最多回退几次？** 需要设置一个上限——如果回退 3 次仍然构建失败，说明模型无法修复，流水线应该终止并报告错误。这个上限在 AgentState 中用 `Channel` + `Reducer` 维护——`retryCount` 使用 `appending_reducer` 累加计数：

```java
// LangGraph4j 条件边中加入回退上限判断
.addConditionalEdges("buildVerification",
    state -> {
        BuildStatus status = state.buildStatus().orElse(BuildStatus.FAILED);
        ...
        if (status == BuildStatus.SUCCESS) {
            ...
            return "codeGeneration";  // 回退，retryCount 由 Reducer 自动累加
        }
    },
    Map.of(
        "previewDeploy", "previewDeploy",
...
```java
@Component
public class BuildVerificationNode {

    private final SandboxService sandboxService;

    /**
     * LangGraph4j NodeAction：返回 Map<String, Object> 更新 AgentState
     */
    public Map<String, Object> execute(CodeGenState state) {
        String projectId = state.projectId().orElseThrow();
        ...
        if (buildResult.isSuccess()) {
            ...
            updates.put("buildStatus", BuildStatus.SUCCESS);
            updates.put("buildTime", buildResult.getBuildTime());
        } else {
            emitSseLog(sink, "构建失败: " + buildResult.getError());
            updates.put("buildStatus", BuildStatus.FAILED);
            updates.put("buildError", buildResult.getError());
            updates.put("retryCount", 1);  // Reducer 会累加到现有值
        }
...
构建验证通过后，预览部署节点启动 Vite Dev Server，把预览 URL 写入 AgentState，通过 SSE 推送给前端。前端拿到 URL 后加载 iframe。

这也是一个**纯逻辑节点**——不需要 LLM，只需要调用 SandboxService 启动 Dev Server。

```java
@Component
public class PreviewDeployNode {
    ...
    public Map<String, Object> execute(CodeGenState state) {
        ...
        return Map.of(
            "previewUrl", sandbox.getUrl(),
            "previewPort", sandbox.getPort()
        );
    }
}
```
...
模型怎么知道 targetFiles？它调用 readFileContext 读取项目的文件树和关键文件内容，自己推断"套餐价格出现在哪些文件里"。这是 `agent → tools → agent` 循环图的自主决策——不需要硬编码"价格在 PlanCard.tsx 里"，模型自己搜索和定位。
...
```java
// LangChain4j 的方式：@Tool 注解方法，归属 IterationTools 类
@Component
public class IterationTools {

    private final ProjectService projectService;

    @Tool("在项目文件中搜索包含指定关键词的代码片段，返回匹配行及上下文")
    public String searchCode(String keyword, List<String> filePaths) {
        String projectId = getCurrentProjectId();
...
## 九、@Tool 工具体系——Agent 的手脚

灵码工坊需要编写 6 个 `@Tool` 方法。每个工具是 Agent 循环图中可以调用的"手脚"——Agent 决定做什么（大脑），工具执行具体操作（手脚）。

**注意：需求分析和执行规划节点不再需要 validateJson 工具。因为使用了 `AiServices` 结构化输出，框架自动保证输出格式正确——不需要手动验证 JSON 是否合法、字段是否完整。这从 7 个工具减少到 6 个。**

**LangChain4j 的 @Tool 设计哲学**：不需要像 Spring AI Alibaba 的 `ToolCallback` 那样实现一个接口（`getName()`、`getDescription()`、`call(String args)`），也不需要手动 `parseArgs(functionArgs)` 做 JSON 反序列化。LangChain4j 只需要在普通 Java 方法上加 `@Tool("描述")` 注解——方法名就是工具名、方法参数就是工具参数、方法返回值就是工具返回值。框架自动把方法签名转成 JSON Schema 给模型看，模型调用工具时框架自动把参数映射到方法参数上。**零手动 JSON 解析，零接口实现。**

### 9.1 完整工具清单

| 工具名 | 归属类 | 哪个 Agent 用 | 做什么 | 实现难度 |
|--------|--------|-------------|--------|---------|
| **writeFile** | FileTools | 代码生成 | 写入完整文件到项目工作区 | 中（双写：磁盘 + 数据库） |
| **readFileContext** | ProjectContextTools | 代码生成、样式优化、迭代修改 | 读取已生成文件的内容作为上下文 | 低（读数据库） |
| **readProjectContext** | ProjectContextTools | 需求分析（可选） | 读取项目框架、文件树、package.json | 低（读数据库） |
| **patchFile** | FileTools | 样式优化、迭代修改 | 增量修改文件（只改指定行） | 中（需要行定位和精确替换） |
| **searchCode** | IterationTools | 迭代修改 | 在文件中搜索关键词，返回匹配行及上下文 | 低（正则匹配 + 行号定位） |
| **validateCode** | FileTools | 代码生成 | 验证代码质量（import 路径、export、类型） | 中（需要解析 import 语句） |

**为什么工具按业务域分组而不是每个工具一个类？**

Spring AI Alibaba 的 `ToolCallback` 是一个接口——每个工具必须单独实现一个类（`WriteFileTool implements ToolCallback`、`ReadFileContextTool implements ToolCallback`……），因为接口只能代表一个工具。LangChain4j 的 `@Tool` 注解加在普通方法上——一个类可以包含多个 `@Tool` 方法。所以 `FileTools` 包含 `writeFile`、`patchFile`、`validateCode` 三个工具，`ProjectContextTools` 包含 `readProjectContext`、`readFileContext` 两个工具，`IterationTools` 包含 `searchCode` 一个工具。

按业务域分组的好处：
1. **减少类的数量**：从 6 个类减少到 3 个类
2. **共享依赖**：`FileTools` 的三个方法共享 `ProjectService` 依赖，不需要在 3 个独立类里分别注入
3. **注册更方便**：`AiServices.builder().tools(new FileTools(...))` 一行注册三个工具
...
**原则四：方法参数使用自解释性命名，而不是缩写。**

`writeFile(String path, String content)` 比 `writeFile(String p, String c)` 好。模型看到 `path` 和 `content` 就知道应该传什么值，看到 `p` 和 `c` 就得猜。LangChain4j 会自动把方法参数名和类型转成 JSON Schema 的字段定义——所以参数名直接影响模型对工具的理解。

**原则五：@Tool 方法不需要手动 JSON 解析。**

这是 LangChain4j 和 Spring AI Alibaba 最大的区别。Spring AI Alibaba 的 `ToolCallback.call(String functionArgs)` 接收一个原始 JSON 字符串，你需要手动 `parseArgs(functionArgs)` 反序列化。LangChain4j 的 `@Tool` 方法参数就是方法参数——框架自动把模型输出的 JSON 参数映射到方法参数上。你不需要写 `parseArgs()`、不需要定义 `WriteFileArgs` / `ReadFileContextArgs` 这些 DTO 类、不需要 `try-catch` 处理 JSON 解析失败。**工具实现的代码量减少了一半以上。**
...
**输出约束**是最重要的层次——但在使用 `AiServices` 接口模式后，这个层次**已经被框架接管了**。灵码工坊的需求分析和执行规划节点不需要在提示词里写"只输出 JSON，不要加前缀文字"——因为 `AiServices` 内部会自动从 Java 类生成 JSON Schema 并注入到模型请求中，模型按照 Schema 输出，框架自动反序列化。**提示词不再承担"约束输出格式"的职责——这个职责交给了 Java 类定义（单一来源）。**

对于代码生成和样式优化节点，输出不是结构化 JSON（而是代码内容或 diff 片段），所以输出约束仍然需要写在提示词里："只输出代码内容，不要用 ```tsx``` 包裹"。但这两个节点的 Agent 循环模式天然解决了这个问题——模型在 Agent 循环中会调用 `writeFile` 或 `patchFile` 工具来输出结果，而不是直接输出文本。工具的调用格式本身就是输出约束。
...
user prompt 的占位符值从 AgentState 获取。每个节点在构建 Agent 的 user prompt 时，从当前状态中提取需要的值：
...
variables.put("description", analysisResult.description());
...
variables.put("analysisResult", analysisResult.toJsonString());
...
**LangChain4j 的模型路由比 Spring AI Alibaba 更直接**——不需要通过 DashScope Starter 的 `spring.ai.dashscope.*` YAML 配置再映射到 `SpringAiAlibabaChatModel` Bean，而是直接用 `OpenAiChatModel.builder()` 或 `AnthropicChatModel.builder()` 创建模型实例。每个模型是一个独立的 Bean，配置方式一目了然。
...
public class ModelRoutingConfig {
    ...
    public ChatModel claudeModel() {
        // Claude Sonnet 4 直接通过 Anthropic 协议接入
...
每个 Agent 通过 `@Qualifier` 注入对应的模型 Bean。换模型只需要改 Bean 定义，不需要改业务代码。**LangChain4j 的优势在于多模型接入的一致性**——不管用什么模型（DeepSeek、Claude、GPT-4、Qwen），创建方式都是 `xxxChatModel.builder()`，接口都是统一的 `ChatModel`。不像 Spring AI Alibaba 那样，DashScope 模型和其他模型有不同的接入方式。
...
3. 执行 Agent（调用模型 + 工具）或直接调用 AiServices
4. 将输出写入 AgentState（返回 `Map<String, Object>`）
5. SSE 推送进度

**但在 LangChain4j + LangGraph4j 的架构下，节点不再继承统一的 `AbstractReactAgentNode` 基类。** 原因是：

- 需求分析和执行规划节点使用 `AiServices` 接口模式（单次调用，无工具循环），它们的执行逻辑是"调用 `analyzer.analyze()` / `planner.plan()` → 写入 State"
- 代码生成和样式优化节点使用 `agent → tools → agent` 循环图（多次推理 + 工具调用），它们的执行逻辑是"构建 StateGraph → stream() → 收集结果"
- 构建验证和预览部署节点是纯逻辑（不调用 LLM），它们的执行逻辑是"调用 SandboxService → 写入 State"

这三类节点的执行逻辑差异太大，强行用一个基类统一会导致抽象过度。**LangGraph4j 的设计哲学是"节点就是一个返回 `Map<String, Object>` 的方法"**——不需要实现 `NodeAction` 接口、不需要继承基类、不需要 `getNodeName()` 方法。每个节点的 `execute` 方法签名就是 `Map<String, Object> execute(CodeGenState state)`，然后通过 `graph.addNode("nodeName", node::execute)` 注册到 StateGraph。

**但模板方法模式仍然有用——只是应用层级变了：**

不再在节点类层级用模板方法，而是在 **Agent 创建层级** 用模板方法。不同节点的 Agent 配置不同（不同模型、不同工具、不同提示词），但创建流程是相同的——"加载 system prompt → 创建 AiServices / 构建 agent → tools → agent 循环图"。这个共同流程可以抽取到一个 `AgentFactory`：
...
public class AgentFactory {
    ...
    /**
     * 创建 AiServices（用于需求分析、执行规划等无工具的结构化输出节点）
     */
    public <T> T createAiService(Class<T> interfaceClass, String modelQualifier) {
        ChatModel model = modelBeans.get(modelQualifier);
        String nodeName = inferNodeName(interfaceClass);
        String systemPrompt = promptLoader.loadSystemPrompt(nodeName);
    ...
    /**
     * 创建 agent → tools → agent 循环图（用于代码生成、样式优化等有工具的节点）
     */
    public StateGraph<CodeGenState> createAgentGraph(
            String nodeName, String modelQualifier, Object... toolObjects) {
        ChatModel model = modelBeans.get(modelQualifier);
        String systemPrompt = promptLoader.loadSystemPrompt(nodeName);

        // 创建带 @Tool 的 AiServices
        CodeGenAgent agent = AiServices.builder(CodeGenAgent.class)
            .chatModel(model)
            .systemMessageProvider(chatMemoryId -> systemPrompt)
            .tools(toolObjects)
        ...
        // 构建循环图
        return new StateGraph<>(CodeGenState.class)
            .addNode("agent", callModel(agent))
            .addNode("tools", invokeTools(agent))
            .addConditionalEdges("agent", routeMessage(agent),
                Map.of("tools", "tools", END, END))
...
不同节点的 Agent 配置不同（不同模型、不同工具、不同提示词）。LangChain4j + LangGraph4j 的 Agent 创建方式比 Spring AI Alibaba 的 `ReactAgent.builder()` 更灵活——它有两种创建模式：

1. **`AiServices` 创建**：用于需求分析、执行规划等不需要工具循环的节点——一行 `AiServices.create(Interface.class, model)` 就够了
2. **`StateGraph` + `AiServices` 创建**：用于代码生成、样式优化等需要 agent → tools → agent 循环的节点——需要构建 StateGraph

集中到一个 `AgentFactory`（和 12.2 节中定义的 `AgentFactory` 是同一个类）：
...
lingma:
  agents:
    requirement-analysis:
      model-qualifier: deepSeekModel
      type: ai-service
      interface-class: com.lingmaforge.ai.RequirementAnalyzer
      # 不需要 tools，AiServices 直接做结构化输出
    execution-planning:
      model-qualifier: deepSeekModel
      type: ai-service
      interface-class: com.lingmaforge.ai.ExecutionPlanner
    code-generation:
      model-qualifier: claudeModel
      type: agent-graph
      tool-classes: [FileTools, ProjectContextTools]  # writeFile, readFileContext, validateCode
    style-optimization:
      model-qualifier: deepSeekModel
      type: agent-graph
      tool-classes: [ProjectContextTools, FileTools]  # readFileContext, patchFile
    iteration-modification:
      model-qualifier: claudeModel
      type: agent-graph
      tool-classes: [ProjectContextTools, FileTools, IterationTools]  # readFileContext, patchFile, searchCode
```

**`type: ai-service` vs `type: agent-graph` 的区别**：

- `ai-service`：用 `AiServices.create()` 创建——单次调用，无工具循环，输出直接是结构化 Java 对象。适用于需求分析、执行规划。
- `agent-graph`：用 `StateGraph` + `AiServices.builder().tools()` 构建 agent → tools → agent 循环图——多次推理 + 工具调用，模型自己决定循环策略。适用于代码生成、样式优化、迭代修改。

**好处**：换模型、加工具、调参数——全部在配置文件里改，不需要改 Java 代码。Agent 的"配方"和 Agent 的"实现"完全分离。

### 12.4 观察者模式（Observer Pattern）——SSE 事件推送

每个节点的执行过程需要通过 SSE 推送给前端。LangGraph4j 的 `graph.stream()` 返回 `AsyncGenerator<NodeOutput>`——每一轮 Agent 循环的输出都是一个 `NodeOutput` 对象，包含节点名称、输出内容、是否包含工具调用等信息。这比 Spring AI Alibaba 的 `ReactAgent` 黑盒模式更透明——你可以从 `NodeOutput` 中清楚地看到每一轮循环发生了什么。

**LangGraph4j 的 stream 和 SSE 推送的结合方式**：

```java
// 在 GenerationService 中，把 graph.stream() 的每一轮输出转为 SSE 事件
public void streamGenerationToSSE(CodeGenState initialState, String taskId) {
    StateGraph<CodeGenState> graph = buildPipelineGraph();
    AsyncGenerator<NodeOutput> stream = graph.stream(initialState);

    CompletableFuture.runAsync(() -> {
        for (var nodeOutput : stream) {
            // 把 NodeOutput 转为 SSE 事件推送给前端
            GraphNodeResponse response = GraphNodeResponse.builder()
                .threadId(taskId)
                .nodeName(nodeOutput.node())
                .text(nodeOutput.output().toString())
                .textType(TextType.TEXT)
                .build();

            sseSink.tryEmitNext(ServerSentEvent.builder(response)
                .event("message").build());
        }
...
这种方式的优点是**所有 SSE 事件都来自 `NodeOutput`**——不需要在每个节点类里手动调用 `emitSseLog()`。`NodeOutput` 是 LangGraph4j 自动生成的，它包含了节点名称、输出内容、是否包含工具调用等信息。你只需要遍历 `AsyncGenerator` 并把每个 `NodeOutput` 转成 SSE 事件。

**对于不需要 Agent 循环的节点（需求分析、执行规划、构建验证、预览部署）**，它们的输出也通过同样的 `graph.stream()` 推送——因为这些节点也是 StateGraph 中的节点，它们的输出同样是 `NodeOutput`。
...
LangGraph4j 的 `AgentState` 是 StateGraph 的状态容器——它像一个"共享黑板"，所有节点都可以从上面读取数据、往上面写入数据。节点之间不直接传递数据，而是通过 AgentState 间接传递。

**AgentState 和 Spring AI Alibaba 的 OverAllState 的核心区别**：

`OverAllState` 是 `Map<String, Object>` ——所有值都是 Object 类型，读取时需要手动转型 `(String) state.value("xxx")`、`(RequirementSpec) state.value("xxx")`。这种设计简洁但类型不安全——如果你把 `analysisResult` 写成了 JSON 字符串，但在代码生成节点里用 `(RequirementSpec)` 读取，就会抛 `ClassCastException`。

LangGraph4j 的 `AgentState` 用 **`Channel` + `Reducer`** 解决了这个问题：

- **`Channel`** 定义了每个状态字段的类型和合并策略。比如 `Channel.<List<GeneratedFile>>of("generatedFiles").reducer(AppendingReducer.INSTANCE)` 声明 `generatedFiles` 是 `List<GeneratedFile>` 类型，合并策略是追加（append）而不是覆盖（overwrite）。
- **`Reducer`** 定义了当多个节点同时写入同一个字段时，怎么合并它们的值。比如 `generatedFiles` 使用 `AppendingReducer`——代码生成节点追加一个文件，样式优化节点追加一个修改记录，两者不会互相覆盖。而 `buildStatus` 使用 `LastValueReducer`（默认）——构建验证节点写入 `FAILED`，覆盖之前可能存在的 `PENDING`。
...
### 13.2 定义 CodeGenState

在灵码工坊中，我们定义一个 `CodeGenState` 类来替代 Spring AI Alibaba 的 `OverAllState + GenerationStateKeys + GenerationStateAccessor` 三件套：

```java
/**
 * 灵码工坊流水线的状态定义
 * 替代 OverAllState + GenerationStateKeys + GenerationStateAccessor 三件套
 *
 * LangGraph4j 的 AgentState 使用 Channel + Reducer 定义每个字段的类型和合并策略
 * - Channel.of().reducer() → 声明字段类型 + 合并方式
 * - AppendingReducer → 追加合并（用于列表类字段）
 * - LastValueReducer → 覆盖合并（默认，用于单值字段）
 */
public class CodeGenState extends AgentState {

    // ===== 输入 =====
    /** 用户原始需求文本 */
    public Optional<String> prompt() { return data().get("prompt"); }
    /** 项目 ID */
    public Optional<String> projectId() { return data().get("projectId"); }
    /** 任务 ID (= SSE streamId) */
    public Optional<String> taskId() { return data().get("taskId"); }

    // ===== 需求分析产出 =====
    /** 需求分析结果（RequirementSpec Java 对象，不是 JSON 字符串） */
    public Optional<RequirementSpec> analysisResult() { return data().get("analysisResult"); }

    // ===== 执行规划产出 =====
    /** 执行规划结果（PlanResult Java 对象） */
    public Optional<PlanResult> planResult() { return data().get("planResult"); }
    /** 当前正在生成的文件序号 */
    public Optional<Integer> currentFileIndex() { return data().get("currentFileIndex"); }

    // ===== 代码生成产出 =====
    /** 已生成的文件列表 */
    public Optional<List<GeneratedFile>> generatedFiles() { return data().get("generatedFiles"); }

    // ===== 构建验证产出 =====
    /** 构建状态: SUCCESS / FAILED */
    public Optional<BuildStatus> buildStatus() { return data().get("buildStatus"); }
    /** 构建错误信息 */
    public Optional<String> buildError() { return data().get("buildError"); }
    /** 构建耗时（秒） */
    public Optional<Integer> buildTime() { return data().get("buildTime"); }
    /** 回退重试次数 */
    public Optional<Integer> retryCount() { return data().get("retryCount"); }

    // ===== 预览部署产出 =====
    /** 预览 URL */
    public Optional<String> previewUrl() { return data().get("previewUrl"); }
    /** 预览端口 */
    public Optional<Integer> previewPort() { return data().get("previewPort"); }
}

/**
 * StateGraph Channel 定义——声明每个字段的类型和合并策略
 */
public class CodeGenStateSchema {

    public static Map<String, Channel<?>> channels() {
        return Map.ofEntries(
            // 输入字段（LastValueReducer：单值覆盖）
            entry("prompt", Channel.of(String.class)),
            entry("projectId", Channel.of(String.class)),
            entry("taskId", Channel.of(String.class)),

            // 需求分析产出（LastValueReducer：单值覆盖）
            entry("analysisResult", Channel.of(RequirementSpec.class)),
            entry("planResult", Channel.of(PlanResult.class)),
            entry("currentFileIndex", Channel.of(Integer.class)),

            // 代码生成产出（AppendingReducer：追加合并）
            // 多个文件生成循环中，每个循环追加一个 GeneratedFile
            entry("generatedFiles", Channel.<List<GeneratedFile>>of()
                .reducer(AppendingReducer.INSTANCE)),

            // 构建验证产出（LastValueReducer：单值覆盖）
            entry("buildStatus", Channel.of(BuildStatus.class)),
            entry("buildError", Channel.of(String.class)),
            entry("buildTime", Channel.of(Integer.class)),
            entry("retryCount", Channel.of(Integer.class)
                .reducer(AppendingReducer.INSTANCE)),  // 每次回退追加 1

            // 预览部署产出（LastValueReducer：单值覆盖）
            entry("previewUrl", Channel.of(String.class)),
            entry("previewPort", Channel.of(Integer.class))
        );
    }
}
```
...
| 变化点 | 旧方式（Spring AI Alibaba） | 新方式（LangGraph4j） |
|--------|---------------------------|---------------------|
| 状态定义 | `OverAllState (Map<String,Object>)` + `GenerationStateKeys` 常量 + `GenerationStateAccessor` 工具类 | `CodeGenState extends AgentState` + `Channel` + `Reducer` |
| 字段类型 | 全部是 `Object`，读取时手动转型 | `Channel.of(RequirementSpec.class)` 声明具体类型 |
| 合并策略 | `state.put()` 简单覆盖，无法区分"追加"和"覆盖" | `AppendingReducer`（追加） vs `LastValueReducer`（覆盖） |
| 类型安全读取 | `(RequirementSpec) state.value("xxx").orElseThrow()` + 手动转型 | `state.analysisResult().orElseThrow()` + 自动类型推断 |
| 工具类数量 | 3 个类（Keys + Accessor + OverAllState） | 2 个类（CodeGenState + CodeGenStateSchema） |

**最关键的变化是 `AppendingReducer`**：代码生成节点每生成一个文件就追加一条 `GeneratedFile` 记录到 `generatedFiles` 字段。在旧方式中，这需要 `state.put("generatedFiles", existingFiles + newFile)` 手动合并。在 LangGraph4j 中，只要声明 `Channel.<List<GeneratedFile>>of().reducer(AppendingReducer.INSTANCE)`，节点只需要返回 `Map.of("generatedFiles", List.of(newFile))`，框架自动追加到已有列表中。**节点不需要知道当前 State 里有什么，只需要返回"我要追加什么"——这让节点代码更简洁、更无状态。**

下面的图展示了每个节点从 State 读取什么、写入什么。你可以把它当作"数据流地图"——知道每个字段从哪里来、到哪里去。

```mermaid
graph LR
    subgraph "AgentState（Channel + Reducer）"
        P["prompt"]
        PID["projectId"]
        AR["analysisResult<br/>RequirementSpec"]
        PR["planResult<br/>PlanResult"]
        CFI["currentFileIndex"]
        GF["generatedFiles<br/>AppendingReducer"]
        BS["buildStatus"]
        BE["buildError"]
        RC["retryCount<br/>AppendingReducer"]
        PU["previewUrl"]
    end
    ...
    subgraph 节点
        ...
        SO["样式优化<br/>读取: GF<br/>写入: GF(appended)"]
...
### 13.4 状态值的类型安全——已经由 Channel 解决

在 Spring AI Alibaba 的 `OverAllState (Map<String, Object>)` 中，所有值都是 Object 类型，读取时需要手动转型 `(RequirementSpec) state.value("xxx")`——这容易出错。但在 LangGraph4j 中，**`Channel` 已经声明了每个字段的类型**，所以类型安全在定义时就保证了，不需要额外的 Accessor 类。

回顾 13.2 节的 `CodeGenState` 定义——每个字段的读取方法已经返回具体的类型：

```java
// LangGraph4j CodeGenState 的类型安全读取（无需 Accessor 类）
public Optional<RequirementSpec> analysisResult() { return data().get("analysisResult"); }
public Optional<PlanResult> planResult() { return data().get("planResult"); }
public Optional<BuildStatus> buildStatus() { return data().get("buildStatus"); }
```

对比 Spring AI Alibaba 的方式：

```java
// Spring AI Alibaba 的手动转型读取（需要 GenerationStateAccessor 类）
public static RequirementSpec getAnalysisResult(OverAllState state) {
    return (RequirementSpec) state.value("analysisResult").orElseThrow(...);
}
```

**LangGraph4j 的方式不需要 `GenerationStateAccessor` 类**——因为 `CodeGenState` 的方法签名已经声明了返回类型（`Optional<RequirementSpec>`），编译器自动做类型检查。如果你把 `analysisResult` 的 `Channel` 类型改成了 `String`，编译器会在所有读取 `state.analysisResult()` 的地方报错，告诉你类型不匹配。这种编译期类型检查比运行期手动转型更安全、更早发现问题。

**整个 State 链路不再有 JSON 字符串中间态，从需求分析到代码生成全是类型安全的 Java 对象传递。**
...
│
├── service/
│   ├── GenerationService.java           # Graph 编排 + SSE 推送（stream → NodeOutput → SSE）
...
├── ai/
│   ├── pipeline/
│   │   ├── CodeGenPipeline.java         # StateGraph 定义（六节点 + 条件边）
│   │   ├── IterationPipeline.java       # 迭代修改 StateGraph
│   │   ├── CodeGenerationGraph.java     # 代码生成 agent→tools→agent 循环图
│   │   ├── CodeGenState.java            # 状态定义（extends AgentState）
│   │   └── CodeGenStateSchema.java      # Channel + Reducer 定义
│   │
│   ├── service/
│   │   ├── RequirementAnalyzer.java     # AiServices 接口（需求分析）
│   │   ├── ExecutionPlanner.java        # AiServices 接口（执行规划）
│   │   └── CodeGenAgent.java            # AiServices 接口（代码生成，带 @Tool）
│   │
│   ├── node/
│   │   ├── RequirementAnalysisNode.java # 需求分析（AiServices 单次调用）
│   │   ├── ExecutionPlanningNode.java   # 执行规划（AiServices 单次调用）
│   │   ├── CodeGenerationNode.java      # 代码生成（agent→tools→agent 循环）
│   │   ├── StyleOptimizationNode.java   # 样式优化（agent→tools→agent 循环）
│   │   ├── BuildVerificationNode.java   # 构建验证
│   │   └── PreviewDeployNode.java       # 预览部署
│   │
│   ├── tool/
│   │   ├── FileTools.java               # writeFile + patchFile + validateCode
│   │   ├── ProjectContextTools.java      # readProjectContext + readFileContext
│   │   └── IterationTools.java           # searchCode
│   │
│   ├── factory/
│   │   ├── AgentFactory.java            # Agent 创建工厂（AiServices / agent→tools→agent）
│   │   └── ModelRoutingConfig.java      # 模型路由配置（LangChain4j Builder）
│   │
│   └── observer/
│       └── SSEProgressObserver.java      # NodeOutput → SSE 推送（从 AsyncGenerator 遍历）
│
├── model/
│   ├── RequirementSpec.java             # 需求分析结构化输出 Record（AiServices 返回类型）
│   ├── PlanResult.java                  # 执行规划结构化输出 Record（AiServices 返回类型）
│   ├── FilePlan.java                    # 单个文件规划 Record
│   ├── GeneratedFile.java               # 已生成文件 Record
│   ├── Patch.java                       # 增量修改 Patch Record
│   ├── BuildResult.java                 # 构建结果 Record
│   ├── BuildStatus.java                 # 构建状态枚举
│   └── GraphNodeResponse.java           # SSE 事件 Record
│
resources/
├── prompts/
...
> **文档版本**: v3.0 | **日期**: 2026-06-26 | **更新**: 全面采用 LangChain4j + LangGraph4j，替换 Spring AI Alibaba 所有代码模式（AiServices 接口模式、@Tool 注解方法、agent→tools→agent 循环图、AgentState + Channel + Reducer）
