...
- [七、后端——基于 Spring AI Alibaba 的技术架构](#七后端基于-spring-ai-alibaba-的技术架构)
...
graph TB
    ...
    subgraph "AI 生成引擎 (Spring AI Alibaba)"
...
- **中间横排**是 AI 生成引擎的六阶段流水线——这是灵码工坊的心脏，由 Spring AI Alibaba 的 StateGraph 编排。
...
StateGraph 是 Spring AI Alibaba 提供的图式工作流引擎。你可以像画流程图一样定义阶段之间的连接关系，包括条件分支——比如"构建验证如果失败，就回退到代码生成"。

```mermaid
stateDiagram-v2
    [*] --> requirementAnalysis: 用户输入需求
    requirementAnalysis --> executionPlanning: 分析完成
    executionPlanning --> codeGeneration: 规划完成
    codeGeneration --> styleOptimization: 代码生成完成
    styleOptimization --> buildVerification: 样式优化完成
    buildVerification --> previewDeploy: ✅ 构建成功
    buildVerification --> codeGeneration: ❌ 构建失败 (自动回退修复)
    previewDeploy --> [*]: 预览就绪

    note right of buildVerification
        这是灵码工坊最关键的设计：
        构建失败不是终点，
        而是回退到代码生成节点
        让 AI 自动修复代码。
...
### 5.3 ReactAgent——每个阶段怎么执行？

四个 AI 阶段（需求分析、执行规划、代码生成、样式优化）使用 **ReactAgent** 驱动。

ReactAgent 是 Spring AI Alibaba 提供的 Agent 模式——它不是"扔给模型一句 prompt 然等结果"，而是让模型进入一个 **Reasoning → Acting → Observing → Reasoning → ...** 的循环：

1. **Reasoning**（思考）：模型看到当前状态，决定下一步做什么
2. **Acting**（行动）：模型调用一个工具（比如 `writeFile` 写文件、`readFileContext` 读已有文件）
3. **Observing**（观察）：工具返回结果，模型看到执行效果
4. **继续循环**：模型根据执行效果继续思考，直到任务完成

这个循环意味着模型不是一次性吐出所有代码，而是**逐步生成**——先读一下项目上下文，再决定生成哪些文件，每个文件生成后验证一下是否合理，然后继续下一个。

**为什么要用 ReactAgent 而不是直接调 ChatModel？**

直接调 ChatModel，你只能发一句 prompt，等模型一次性返回。如果模型输出格式不对，你只能重新发 prompt 重试。ReactAgent 让模型自己决定什么时候调用工具、什么时候输出内容、什么时候结束——**模型变成了一个有自主判断能力的执行者，而不是一个被动响应的问答引擎。**
...
所有模型通过 **Spring AI Alibaba 的 DashScope Starter** 统一接入——通义千问、DeepSeek、Claude，一行配置切换，代码层面只是换了 `@Qualifier` 注解引用的 Bean。
...
- SSE 事件的 `nodeName` 字段驱动 UI 映射：`RequirementAnalysisNode` → 清单项"需求分析"亮起
...
- 随 SSE `plan` 事件初始化骨架，随 `codeGeneration` 的 `AGENT_MODEL_STREAMING` 事件逐个添加
...
- SSE `AGENT_MODEL_STREAMING` 事件实时注入代码
...
## 七、后端——基于 Spring AI Alibaba 的技术架构

### 7.1 为什么选 Spring AI Alibaba？

灵码工坊的后端核心围绕 AI 展开——LLM 接入、Agent 编排、流式推送、状态管理。这些能力传统上需要两个框架协作：Spring AI 做 LLM 接入、LangChain4j 做 Agent 编排。

但 Spring AI Alibaba v1.1+ 把这三层能力**合进了一个框架**：

| 层级 | 能力 | 传统做法 | Spring AI Alibaba 做法 |
|------|------|---------|----------------------|
| **接入层** | LLM 统一接入 | Spring AI + 手动配置各模型 | DashScope Starter 一行配置，通义/DeepSeek/Claude 全覆盖 |
| **编排层** | 工作流图式编排 | LangChain4j SequentialAgent 或自写循环 | StateGraph + 条件边 + 子图嵌套 |
| **Agent 层** | 自主推理执行 | LangChain4j 的 AiService 或自写 ReAct 循环 | ReactAgent 内置 Reasoning-Acting 循环 |

**一个框架覆盖三层，意味着：**
- 只有一套 BOM 版本管理，不会出现 Spring AI 1.1 和 LangChain4j 0.36 的版本冲突
- 国产模型（通义千问、DeepSeek）是"一等公民"，不需要适配层
- StateGraph 和 ReactAgent 是同一个生态的组件，互操作零成本
...
| **AI 核心** | Spring AI BOM | 1.1.2 | AI 抽象层版本管理 |
| **AI 核心** | Spring AI Alibaba BOM | 1.1.2.2 | Agent + Graph + DashScope 版本管理 |
| **AI 接入** | spring-ai-alibaba-starter-dashscope | — | LLM 统一接入（通义/DeepSeek/Claude） |
| **AI 编排** | spring-ai-alibaba-graph-core | — | StateGraph 图式工作流 |
| **AI Agent** | spring-ai-alibaba-agent-framework | 1.1.2.0 | ReactAgent + 上下文工程 + 流式 |
...
<project xmlns="http://maven.apache.org/POM/4.0.0"
    ...
    <properties>
        ...
        <!-- AI 框架版本（对齐 Spring AI Alibaba 最新） -->
    ...
    <dependencies>
        ...
        <!-- DashScope 模型接入（通义千问 + DeepSeek + 兼容 OpenAI） -->
        <dependency>
            <groupId>com.alibaba.cloud.ai</groupId>
            <artifactId>spring-ai-alibaba-starter-dashscope</artifactId>
        </dependency>

        <!-- Agent Framework: ReactAgent + 上下文工程 + 流式输出 -->
        <dependency>
            <groupId>com.alibaba.cloud.ai</groupId>
...
spring:
  ...
  ai:
    ...
    # --- DashScope（通义千问系列 / DeepSeek 兼容模式）---
    dashscope:
      api-key: ${DASHSCOPE_API_KEY}

    # --- OpenAI 兼容模式（DeepSeek / Claude / 其他兼容服务）---
    openai:
...
lingma:
  # 代码生成流水线模型路由
  pipeline:
    analysis-model: deepseek-chat        # 需求分析
    planning-model: deepseek-chat        # 执行规划
    code-gen-model: claude-sonnet-4-20250514  # 代码生成（主力）
    style-model: deepseek-chat           # 样式优化
...
| **lingmaforge-ai** | CodeGenPipeline / 各阶段 Agent / 自定义 ToolCallback | StateGraph 构建、ReactAgent 定义、工具注册 |
...
sequenceDiagram
    ...
    Graph-->>Frontend: SSE event | nodeName: RequirementAnalysis | 结构化 JSON 规范

    Note over Graph,Frontend: 第二阶段：执行规划
    Graph-->>Frontend: SSE event | nodeName: ExecutionPlanning | 文件树 + API 清单

    Note over Graph,Frontend: 第三阶段：代码生成（逐文件）
    loop 每个文件
        Graph-->>Frontend: SSE event | nodeName: CodeGeneration | AGENT_MODEL_STREAMING
        Note over Frontend: FileTree 添加节点<br/>Editor 实时注入代码
    end

    Note over Graph,Frontend: 第四阶段：样式优化
    Graph-->>Frontend: SSE event | nodeName: StyleOptimization | CSS diff
    ...
    Graph-->>Frontend: SSE event | nodeName: PreviewDeploy | 预览 URL
...
后端使用 `Sinks.Many<ServerSentEvent<GraphNodeResponse>>` 创建一个 unicast 流——一个 SSE 连接对应一个 Sink，一个 Sink 对应一次 Graph 执行。
...
public class GenerationService {
    ...
    public Flux<ServerSentEvent<GraphNodeResponse>> streamGeneration(
        ...
        // 4. 异步启动 Graph 流式执行
        Flux<NodeOutput> graphStream = pipeline.getCompiledGraph().stream(inputs, config);

        Disposable subscription = graphStream
            .doOnNext(nodeOutput -> handleNodeOutput(nodeOutput, sink, taskId))
            .doOnComplete(() -> handleStreamComplete(sink, taskId))
            .doOnError(error -> handleStreamError(sink, taskId, error))
            .doOnCancel(() -> stopStreamProcessing(taskId))
            .subscribe();

        context.setDisposable(subscription);
     ...
     *
     * 关键：根据 OutputType 分流
     * - AGENT_MODEL_STREAMING → 模型"打字机"片段
     * - AGENT_MODEL_FINISHED → 阶段完成 / 工具调用请求
     * - AGENT_TOOL_FINISHED → 工具执行结果
     */
            ...
            switch (type) {
                case AGENT_MODEL_STREAMING -> {
                    ...
                    if (message instanceof AssistantMessage am) {
                        // DeepSeek thinking 与普通内容分离
                        Object reasoning = am.getMetadata().get("reasoningContent");
                        String text = (reasoning != null)
                            ? "[Thinking] " + reasoning : am.getText();
                        emitMessage(sink, taskId, streamingOutput.getNodeName(), text, TextType.TEXT);
                    }
                }
                case AGENT_MODEL_FINISHED -> {
                    ...
                    if (message instanceof AssistantMessage am && am.hasToolCalls()) {
                        am.getToolCalls().forEach(tc ->
                            emitMessage(sink, taskId, "ToolCall",
                                "调用工具: " + tc.name(), TextType.TEXT));
                    }
                }
                case AGENT_TOOL_FINISHED -> {
                    ...
                    if (message instanceof ToolResponseMessage trm) {
                        trm.getResponses().forEach(r ->
                            emitMessage(sink, taskId, "ToolResult",
                                "工具完成: " + r.name(), TextType.TEXT));
                    }
                }
            }
        }
...
export interface SSEMessage {
  threadId: string
  nodeName: string        // "RequirementAnalysisNode" / "CodeGenerationNode" 等
  text: string            // 增量内容
...
| SSE Event | OutputType | 前端做什么 |
|-----------|-----------|-----------|
| `message` | `AGENT_MODEL_STREAMING` | ChatBubble 打字机追加内容 |
| `message` | `AGENT_MODEL_FINISHED` | 清单项标记完成 / 工具调用展示 |
| `message` | `AGENT_TOOL_FINISHED` | 工具执行结果展示 |
| `complete` | — | 六阶段全绿 + 加载预览 |
| `error` | — | 错误提示 + 重试按钮 |

`nodeName` 字段是映射的关键——`RequirementAnalysisNode` → 清单项"需求分析"亮起，`CodeGenerationNode` → 清单项"代码生成"亮起 + 文件树开始长节点。
...
| 3 | ModelRoutingConfig Bean 定义 | 第五章 5.4 |
| 4 | GenerationState 键定义 | 第五章 5.2 |
| 5 | CodeGenPipeline StateGraph 构建 | 第五章 5.2 |
| 6 | RequirementAnalysisNode ReactAgent | 第五章 5.3 |
| 7 | CodeGenerationNode ReactAgent | 第五章 5.3 |
...
| 风险 | 怎么应对 |
|------|---------|
| LLM 输出 JSON 解析失败 | ReactAgent 的 Reasoning-Acting 循环自动重试 |
| 代码质量不达标 → 构建失败 | StateGraph 条件回退 → 自动回退到代码生成修复 |
| Docker 资源消耗 | 内存/CPU 限制 + 超时自动回收 |
| SSE 连接中断 | Redis Checkpoint → 重连后从断点恢复 |
...
| 决策 | 选择 | 一句话理由 |
|------|------|-----------|
| **AI 框架** | Spring AI Alibaba 全家桶 | 一个框架覆盖接入 + 编排 + Agent |
| **工作流编排** | StateGraph + 条件边 | 构建失败可自动回退修复 |
| **Agent 模式** | ReactAgent | 内置推理-执行循环，无需手写 Agent 逻辑 |
| **流式推送** | SSE (Sinks.Many.unicast) | 单客户端流，DataAgent 成熟模式 |
| **状态持久化** | Redis Checkpoint | 断点续推、Human-in-the-Loop |
| **主力代码模型** | Claude Sonnet 4 | 代码质量业界最高 |
| **辅助模型** | DeepSeek-V3 (DashScope 兼容模式) | 中文强、成本低、一行配置切换 |
...
> **文档版本**: v3.0 | **日期**: 2026-06-26
> **定位**: 教学项目第一篇文章——项目全景 + 核心架构规划
> **参考**: [Spring AI Alibaba 官方文档](https://github.com/alibaba/spring-ai-alibaba) · [AI 框架技术选型深度对比](./灵码工坊-AI框架技术选型深度对比.md) · [产品需求文档 PRD](./灵码工坊-产品需求文档PRD.md)
